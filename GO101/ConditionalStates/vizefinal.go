...
/*
	ORTALAMA VİZE NOTUNUN %40 İLE FİNAL NOTUNUN %60 İLE BULUNUR.
...
fmt.Print("Vize sınav notunu giriniz: ")
scanner.Scan()
vizenot,_:=strconv.ParseFloat(scanner.Text(),64)


fmt.Print("Final sınav notunuzu giriniz: ")
scanner.Scan()
finalnot,_:=strconv.ParseFloat(scanner.Text(),64)

ortalama:=(vizenot* 0.4)+ (finalnot*0.6)

fmt.Printf("Vize notu: %.2f Final notu: %.2f Ortalama: %.2f", vizenot,finalnot,ortalama)
